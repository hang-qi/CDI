...
def clean(words):
    ...
    words = [_morphy(w) for w in words]
    cleaned_words = _remove_stopwords(words)
...
def _remove_stopwords(words):
    ...
    for i in range(0, len(words)):
        if (i-1) >= 0 and words[i-1] == 'united' and words[i] == 'states':
            ...
            filtered_words.append(_filter(words[i]))
