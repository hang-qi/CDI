...
def renameHTMLTranscript(oldFilename, newFilename):
    if not os.path.exists(newFilename):
        #os.system("cp -p {src} {tgt}".format(src=oldFilename, tgt=newFilename))
...
def matchAll(htmlTranscriptFiles):
    ...
    for originalHTMLTranscript in htmlTranscriptFiles:
        ...
        print('-' * 20)
        print('For ' + originalHTMLTranscript.replace(os.path.dirname(originalHTMLTranscript),''))
        
        try:
            matchedFilenames = matchfile.getMatchedFilenames(originalHTMLTranscript)        
            ...
            if not os.path.exists(targetCaptionFilename):
                print('[MISSING ALERT] Caption missing at {0}'.format(targetCaptionFilename));
            else:
                print('[GOOD] Caption found at {0}'.format(targetCaptionFilename))
                captionExist = True;

            # rename HTML transcript file
            transcriptExist = renameHTMLTranscript(originalHTMLTranscript, targetHTMLFilename)
